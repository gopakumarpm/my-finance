...
# ======================== GOLD PORTFOLIO (Unified) ========================
    ...
    with gt_physical:
        ...
        # Auto-update price when weight or purity changes
        auto_price = round(gp_weight * rate_map.get(gp_carat, r22), 2) if latest_rate and gp_weight > 0 else 0.0
        _prev_key = f"_gpp_prev_{gpv}"
        _cur = (gp_weight, gp_carat)
        if st.session_state.get(_prev_key) != _cur:
            st.session_state[f"gpp_pr_{gpv}"] = auto_price
            st.session_state[_prev_key] = _cur
    ...
    with gt_digital:
        ...
        _dg_prev_key = f"_dgp_prev_{gdv}"
        if st.session_state.get(_dg_prev_key) != dg_weight:
            st.session_state[f"dgp_pr_{gdv}"] = dg_auto_price
            st.session_state[_dg_prev_key] = dg_weight
        with dc[1]:
